<div align="center">
<img src="assets/logo.svg" width="92" alt="Keural" />

# Keural Desktop

### 내 컴퓨터의 파일을, AI 와 함께.

큐럴 데스크톱 앱은 당신의 로컬 파일을 **직접 읽고, 검색하고, 수정**합니다.
업로드도, 복사·붙여넣기도 필요 없습니다 — 폴더 하나만 열어주면 됩니다.

<br/>

[![Download](https://img.shields.io/badge/⬇_Download-874dff?style=for-the-badge)](https://github.com/MKD-CORP/keural-desktop/releases/latest)
[![Latest Release](https://img.shields.io/github/v/release/MKD-CORP/keural-desktop?style=for-the-badge&color=3382ff&label=latest)](https://github.com/MKD-CORP/keural-desktop/releases/latest)

</div>

---

## 다운로드

최신 버전은 **[GitHub Releases](https://github.com/MKD-CORP/keural-desktop/releases/latest)** 에서 받으세요.

| 플랫폼 | 아키텍처 | 다운로드 |
|---|---|---|
| 🍎 **macOS** | Apple Silicon (M1 이상) | [Keural-1.0.2-arm64.dmg](https://github.com/MKD-CORP/keural-desktop/releases/latest/download/Keural-1.0.2-arm64.dmg) |
| 🍎 **macOS** | Intel | [Keural-1.0.2-x64.dmg](https://github.com/MKD-CORP/keural-desktop/releases/latest/download/Keural-1.0.2-x64.dmg) |
| 🪟 **Windows** | 64-bit | [Keural-Setup-1.0.2.exe](https://github.com/MKD-CORP/keural-desktop/releases/latest/download/Keural-Setup-1.0.2.exe) |
| 🐧 **Linux** | x86_64 | _준비 중_ |

## 설치

### macOS

1. `.dmg` 를 열고 **Keural** 을 응용 프로그램 폴더로 드래그
2. **"손상되었기 때문에 열 수 없습니다"** 가 뜨면:
   ```bash
   xattr -cr /Applications/Keural.app
   ```
3. 또는 차단되면 **시스템 설정 → 개인정보 보호 및 보안 → "확인 없이 열기"**

### Windows

1. `.exe` 실행
2. SmartScreen 경고 시 **추가 정보 → 실행**

### Linux

```bash
chmod +x Keural-linux-x86_64.AppImage
./Keural-linux-x86_64.AppImage
```

---

<div align="center"><sub>© MKD · Keural Desktop</sub></div>
